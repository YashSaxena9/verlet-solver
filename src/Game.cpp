...
Game::Game(int screenWidth, int screenHeight, int frameRate)
...
void Game::SpawnFixedParticles(const std::initializer_list<Vector2>& positions, int particleRadius) {
    m_engine.EnsureCapacity(positions.size()); // optional: it prevents frequent resize
    for (const Vector2& position : positions) {
        m_engine.AddFixedParticle(position, particleRadius, GRAY);
    }
}

void Game::SpawnParticles(const float probability, uint32_t limit) {
    assert(probability <= 1.0f);
    m_engine.EnsureCapacity(limit); // optional: it prevents frequent resize
    const uint32_t particleDiameter = Constants::PARTICLE_RADIUS * 2;
    const uint32_t columns = m_screenWidth / particleDiameter;
    const uint32_t rows = m_screenHeight / particleDiameter;
    ...
    /// BUT, I need to break particle creation at limit and I want it to fill
    ...
    for (uint32_t row = 0; row < rows && actualLimit > 0; row += 1) {
        for (uint32_t col = 0; col < columns && actualLimit > 0; col += 1) {
            ...
            Vector2 generatedPosition = Vector2 {
                (float)(col * particleDiameter) + Constants::PARTICLE_RADIUS,
                (float)(row * particleDiameter) + Constants::PARTICLE_RADIUS
            };
